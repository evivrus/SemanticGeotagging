    ...
    $.getJSON('/api/entities.js?callback=?', function(data) {
  	for(var i=0; i<data.length; i++)
  	{
  		...
  		if(markerExists == false)
  		{
            ...
            markerArrayCopy[i].html = '<div style="font-size: 30px;line-height: 38px;" onClick="popup_open(' + data[i].entity.id + "); return false;\"><a id=\"popup_link\" href=\"#\" ><b>" + data[i].entity.title + '</b><br/>' + data[i].entity.description + '<br/>' + "</a></div>";
